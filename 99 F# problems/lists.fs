...
let removeAt idx list: char * char list =
    let rec loop kthEl acc currentIdx = function
        | [] -> (kthEl, acc |> reverse)
        | hd::tl when currentIdx = idx -> loop hd acc (currentIdx+1) tl
        | hd::tl -> loop kthEl (hd::acc) (currentIdx+1) tl
    if idx >= (list |> myLength) then failwith "index out of bounds"
    else loop 'a' [] 0 list